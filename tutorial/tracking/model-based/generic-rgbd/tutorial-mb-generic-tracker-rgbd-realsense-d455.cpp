...
//选择使用深度密度还是深度法线
  if (use_depth)
    trackerTypes.push_back(vpMbGenericTracker::DEPTH_DENSE_TRACKER);//DEPTH_DENSE_TRACKER
      ...
      if (!tracking_failed) {
        ...
        }
        {
          ...
          ss << "Features: edges " << tracker.getNbFeaturesEdge() << ", klt " << tracker.getNbFeaturesKlt()
            << ", depth " << tracker.getNbFeaturesDepthDense();//<< ", depth " << tracker.getNbFeaturesDepthDense();
